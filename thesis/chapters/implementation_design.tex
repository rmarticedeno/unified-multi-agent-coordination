...
\section{Crash-Fault-Tolerance Extension}

The current prototype records failures as structured results and includes a small JSONL coordination ledger for session recovery. A fault-tolerant implementation should preserve the same logical coordination authority while removing the coordinator process as the sole availability point. The extension is therefore limited to crash fault tolerance: it addresses coordinator process failure, transient registry failure, task timeout, and duplicate dispatch risk. It does not claim Byzantine fault tolerance or agreement among mutually suspicious coordinators.

The first extension is a bounded retry policy. Registry refresh and task dispatch should be retried only when the failure class is transient and the retry budget has not been exhausted. Retry attempts must be recorded as task or registry observations rather than hidden inside the transport layer. When retries are exhausted, the coordinator records the condition as a registry failure, timeout, unavailable agent, or failed commitment. If another compatible agent exists in the same authorized registry snapshot, the coordinator may construct a new candidate plan and submit it to the feasibility analyzer before dispatching replacement work. If no replacement satisfies the same capability, trust, modality, and validation constraints, the run terminates with an explicit failure state.

The second extension is a durable coordination ledger. Each coordination attempt is assigned a stable session identifier. Authorized plans receive a stable plan identifier, and delegated tasks carry stable task identifiers and attempt identifiers. The current JSONL ledger stores the admitted request, registry snapshot used for authorization, candidate proposal, feasibility report, authorization decision, task attempts, runtime results, artifacts, and terminal result. This ledger becomes the authoritative recovery source for simple session resume; natural-language explanations remain derived views rather than the state used for recovery. A fuller implementation should add explicit commitment-state records, lease ownership, and cross-process duplicate-dispatch handling.

The third extension is hot standby recovery. Multiple coordinator processes may be deployed, but only one process should hold the active lease for a given session. If the active coordinator fails, a standby coordinator reads the durable ledger, reconstructs the last authorized plan and commitment states, and resumes only the work that is not already terminal. The prototype can resume from a ledger and skip completed tasks, but it does not yet implement lease management or container/process failover. Completed or failed task attempts must not be blindly repeated. This requires idempotent dispatch semantics: each outbound delegation includes the session, plan, task, and attempt identifiers, and the SDK or receiving agent must be able to detect duplicate requests or return the already recorded result when repetition is unsafe.

This extension keeps the theory's centralized authorization semantics intact. The system may have several coordinator processes, but the session still has one logical authorization record and one current commitment state. Fault tolerance is therefore an implementation property of state durability, retry discipline, and failover, not a shift to decentralized consensus.
