...
For deployment and reproducibility, the repository includes a thin HTTP service and Docker configuration around the same \texttt{CoordinationAgent} and \texttt{CoordinationSdk} objects. The service exposes health, registry, plan, coordinate, resume, and feasibility endpoints. It does not bypass the SDK boundary or change the theoretical ownership of planning and authorization. The implementation can run with the JSONL-backed local store used by simple tests, or with a PostgreSQL-backed shared coordination store selected through \texttt{COORDINATION\_STORE\_URL}. The PostgreSQL mode supports replicated coordinator processes by combining per-session leases, fencing tokens, idempotency keys, and durable terminal-result constraints.
...
\begin{itemize}
  ...
  \item \texttt{CoordinationStore} materializes durable coordination state. Its JSONL adapter preserves the local testing backend; its PostgreSQL adapter stores sessions, events, leases, registry snapshots, plan generations, commitments, attempts, artifacts, and terminal results.
...
\section{Lease-Backed Replicated Coordinator Execution}

The implemented crash-fault-tolerance path preserves the same logical coordination authority while allowing several coordinator processes to serve different sessions and to recover a session after the previous owner stops making progress. The key design point is that replication applies to the coordinator process, not to the authorization role. Each session still has one current lease holder, one authorized plan state per generation, one task-attempt history, and at most one terminal result.

\begin{table}[htbp]
\centering
\caption{Scope of implemented coordination evidence.}
\label{tab:implementation-scope}
\small
\begin{tabular}{p{0.26\textwidth}p{0.30\textwidth}p{0.32\textwidth}}
\toprule
\textbf{Level} & \textbf{Implemented evidence} & \textbf{Claim boundary} \\
\midrule
Local prototype & JSONL-backed sessions, deterministic feasibility checks, SDK dispatch, trace reconstruction, and resume from stored events. & Shows the language--symbolic boundary and recovery for simple local sessions. \\
Replicated coordinator & PostgreSQL store, leases, fencing tokens, idempotency keys, per-attempt provenance, stale-write rejection, HTTP \texttt{409} lease conflict responses, and Docker failover evidence. & Supports crash-fault-tolerant replicated coordinator execution for the measured cases. \\
Decentralized or adversarial coordination & Not implemented: no consensus protocol, adversarial registry defense, malicious Agent Card validation, or Byzantine coordinator tests. & Remains outside scope and is not claimed. \\
\bottomrule
\end{tabular}
\end{table}

The \texttt{CoordinationStore} interface separates durable coordination state from the coordination algorithm. The JSONL adapter keeps local tests and simple demonstrations lightweight. The PostgreSQL adapter uses transactional tables for sessions, leases, append-only events, registry snapshots, plan generations, task commitments, task attempts, artifacts, and terminal results. Its schema enforces key invariants: a session has at most one terminal result, task results require a prior attempt record, stale fencing tokens are rejected, and idempotency keys can be resolved before retrying a dispatch.

Coordinator replicas identify themselves with \texttt{COORDINATION\_COORDINATOR\_ID}. Before planning, authorization, dispatch, recovery, or terminal writes, \texttt{coordinate} and \texttt{resume\_session} acquire a session lease. The store assigns a monotonically increasing fencing token whenever a lease is acquired after expiry or release. Every fenced write carries the lease token, and the PostgreSQL backend rejects stale tokens after takeover. Service endpoints translate live lease conflicts into HTTP \texttt{409}, making ownership contention visible to clients rather than silently reassigning the session.

Dispatch is idempotency-aware. Each attempt uses the key \texttt{session\_id + plan\_id + task\_id + attempt\_id}; the key is persisted before SDK dispatch and is included in the task payload's \texttt{\_coordination} metadata together with the coordinator id, fencing token, and plan generation. The SDK caches completed local and linguistic results by idempotency key, and the deterministic A2A fixture agent returns a prior result when the same key is observed again. The \texttt{CapabilityRequirement.side\_effect\_class} field distinguishes \texttt{read\_only}, \texttt{idempotent}, \texttt{unsafe}, and \texttt{unknown} work. Automatic redispatch is limited to read-only or idempotent attempts. When takeover observes an expired in-flight task whose side effects are unknown or unsafe, the coordinator records an \texttt{unknown} task result and fails explicitly rather than blindly repeating external work.

Recovery reconstructs session state from the store. Completed tasks are skipped, pending tasks whose dependencies are satisfied can resume, and terminal results are returned without replanning. Replanning is deliberately conservative: a new plan generation must be recorded rather than silently mutating the old authorized plan, and it is only justified when the prior task lacks a terminal result, the original executor is unavailable, a compatible replacement exists in a fresh registry snapshot, and the feasibility analyzer accepts the new proposal. The implemented Docker evidence covers lease conflict, takeover after an abandoned attempt, process exit after external dispatch of unknown-side-effect work, stale fencing rejection, and PostgreSQL invariant checks; broader dynamic-registry replanning remains a future evaluation case.

This design keeps the theory's centralized authorization semantics intact. The system may have several coordinator processes, but the session still has one logical authorization record and one current commitment state. Fault tolerance is therefore an implementation property of state durability, lease ownership, retry discipline, idempotency, and failover. It is not a shift to decentralized consensus and does not address Byzantine coordinators or malicious registries.

\section{Chapter Synthesis}

The design specifies a distributed multi-agent environment with a clear split between coordination and communication. \texttt{CoordinationAgent} owns planning, feasibility, authorization, auxiliary decisions, and aggregation. \texttt{CoordinationSdk} owns A2A interoperability, local runtime invocation, Lingo runtime invocation, registry normalization, task dispatch, artifact normalization, communication failures, and trace access. Agents remain independently deployable and vendor independent. A2A provides the interoperability backbone through Agent Cards and task exchange. Local Python agents and Lingo-backed linguistic agents are wrapped by the SDK so that they participate through the same registry, dispatch, artifact, and trace mechanisms as remote A2A agents. The evaluation in Chapters~\ref{chap:methodology} and~\ref{chap:results} examines whether the implementation preserves both communication-boundary compliance and coordination-ownership compliance in practice.
